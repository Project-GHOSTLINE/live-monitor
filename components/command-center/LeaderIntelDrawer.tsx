...
interface LeaderIntelDrawerProps {
  isOpen: boolean;
  onClose: () => void;
  leader: {
    code: string;
...
export function LeaderIntelDrawer({
  isOpen,
  onClose,
  leader,
  power,
  ...
  const handleTouchStart = (e: React.TouchEvent) => {
    ...
    if (target.closest('.drawer-header')) {
      setStartY(e.touches[0].clientY);
    }
  };

  // Handle touch move for swipe down
  ...
  return (
    <>
      ...
      <div
        ...
        className={`fixed bottom-0 left-0 right-0 bg-black/95 border-t-2 border-green-500 shadow-2xl shadow-green-500/40 font-mono text-xs backdrop-blur-sm z-50 transition-transform duration-300 md:hidden ${
      ...
      >
        ...
        {/* Swipe indicator */}
        <div className="drawer-header relative border-b border-green-500/30 p-3 bg-green-950/20 cursor-grab active:cursor-grabbing">
          <div className="w-12 h-1 bg-green-500/40 rounded-full mx-auto mb-3" />
          <div className="flex items-center justify-between">
            ...
            <button
              onClick={onClose}
              className="ml-4 text-green-400 hover:text-red-400 text-xl font-bold transition-colors"
            >
              ✕
            </button>
          </div>
        ...
        {/* Scrollable Content */}
        <div className="relative p-3 space-y-3 overflow-y-auto" style={{ maxHeight: 'calc(80vh - 100px)' }}>
          {/* POWER SECTION */}
          {power && (
            <section className="border border-green-500/30 rounded p-2 bg-green-950/10">
              ...
              {/* AIR/SEA/LAND Grid - Stacked on mobile */}
                ...
                <div className="space-y-1.5">
                  {pulse.latest_items.slice(0, 3).map((item, idx) => (
                    <a
                      ...
                      className="block p-1.5 bg-black/40 rounded border border-green-500/20 hover:border-green-500/60 hover:bg-green-950/20 transition-colors active:bg-green-950/30"
                    >
                      <div className="text-green-400 line-clamp-2">{item.title}</div>
                    ...
                    <div key={idx} className="bg-black/40 p-1.5 rounded border border-green-500/20">
                      ...
                      {signal.evidence.slice(0, 2).map((ev, evIdx) => (
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="block text-green-500/60 hover:text-green-400 active:text-green-300 text-[9px] line-clamp-1 transition-colors"
                        >
                          → {ev.title} ({ev.source})
                        </a>
                      ))}
